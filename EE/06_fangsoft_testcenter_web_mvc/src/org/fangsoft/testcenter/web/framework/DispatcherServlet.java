...
public class DispatcherServlet extends HttpServlet implements javax.servlet.Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        request2ActionMap =(Map<String,ActionConfig>)context.
                getAttribute(Constants.APP_REQ_2_ACTION_CONFIG);
    }
    ...
    protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(index!=-1)requestPath=requestPath.substring(index+1);
        ActionConfig actionConfig=request2ActionMap.get(requestPath);
        ResponsePage responsePage=actionConfig.
                getAction().doProcess(request, response, actionConfig);
        if(ResponsePage.SendMode.REDIRECT==responsePage.getMode()){
            response.sendRedirect(responsePage.getResponseURI());
        }else{
            request.getRequestDispatcher(responsePage.getResponseURI()).
